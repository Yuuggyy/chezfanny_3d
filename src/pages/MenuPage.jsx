  ...
  return (
    <div style={{ minHeight: '100dvh', background: C.cream, fontFamily: "'Inter', sans-serif" }}>
      ...
      <header style={{
        ...
        boxShadow: '0 2px 16px rgba(123,58,90,0.25)',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: isMobile ? 20 : 24, fontWeight: 700, color: C.beige, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {parametres?.nom_restaurant || 'Chez Fanny'}
            </h1>
        ...
        <div style={{ display: 'flex', gap: isMobile ? 8 : 12, flexShrink: 0 }}>
          <button onClick={() => setShowAppel(true)} style={{ background: 'rgba(255,255,255,0.10)', border: `1px solid rgba(232,160,180,0.40)`, color: C.gold, borderRadius: 8, padding: isMobile ? '7px 12px' : '8px 18px', fontSize: 13, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
          ...
          <button onClick={() => setShowPanier(true)} style={{
            ...
            border: totalItems > 0 ? 'none' : `1px solid rgba(232,160,180,0.40)`,
      ...
      <div style={{ position: 'sticky', top: isMobile ? 64 : 72, zIndex: 99, background: C.primaryMid, padding: isMobile ? '10px 16px' : '10px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.10)', border: `1px solid rgba(232,160,180,0.25)`, borderRadius: 10, padding: '9px 14px', maxWidth: isMobile ? '100%' : 520, margin: '0 auto' }}>
      ...
      {/* ── MODAL APPEL ── */}
      {showAppel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(123,58,90,0.50)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onClick={() => setShowAppel(false)}>
      ...
      {/* ── TOAST ── */}
      {toast && <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: C.primary, color: C.beige, padding: '12px 24px', borderRadius: 12, fontSize: 14, fontWeight: 600, zIndex: 200, border: `1px solid ${C.gold}`, boxShadow: '0 8px 24px rgba(123,58,90,0.25)' }}>{toast}</div>}
    </div>
  );
}

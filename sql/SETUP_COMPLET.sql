...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU CHEZ FANNY (cuisine congolaise authentique)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Plats Congolais','Specialites congolaises traditionnelles','🇨🇩',1,true),
('Viandes et Grillades','Viandes braisees et grillees au feu','🥩',2,true),
('Poissons','Poissons locaux prepares a la congolaise','🐟',3,true),
('Buffet du Jour','Formule buffet avec entree et plat','🍽️',4,true),
('Accompagnements','Riz, fufu, plantain, kwanga','🍚',5,true),
('Boissons','Boissons fraiches et locales','🥤',6,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Soso ya Mwamba','Poulet au mwamba - plat national congolais',12.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,1),
('Pondu au Poisson Fume','Feuilles de manioc, poisson fume, huile de palme',10.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,2),
('Makayabu','Morue salee preparee a la congolaise, tomates, piment',11.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,3),
('Fufu au Gombo','Fufu de manioc, sauce gombo avec poisson',8.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,4),
('Liboke de Capitaine','Capitaine cuit en feuilles de bananier, epices',14.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,5),
('Saka-saka Viande','Feuilles de manioc mijotees avec viande de boeuf',12.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,6),
('Poulet Braise Entier','Poulet marine aux epices, braise au feu de bois',14.00,(SELECT id FROM categories WHERE nom='Viandes et Grillades'),true,1),
('Ngolo Braise','Porc braise, sauce locale, piment',13.00,(SELECT id FROM categories WHERE nom='Viandes et Grillades'),true,2),
('Cotes de Porc Grillees','Cotes de porc grillees, marinade citron-epices',12.00,(SELECT id FROM categories WHERE nom='Viandes et Grillades'),true,3),
('Brochettes de Boeuf','Brochettes marinees, grillees, legumes',11.00,(SELECT id FROM categories WHERE nom='Viandes et Grillades'),true,4),
('Tilapia Braise','Tilapia entier braise, citron, piment',12.00,(SELECT id FROM categories WHERE nom='Poissons'),true,1),
('Capitaine Frit','Capitaine frit entier, sauce tomate-piment',14.00,(SELECT id FROM categories WHERE nom='Poissons'),true,2),
('Ndakala','Petits poissons frits croustillants',8.00,(SELECT id FROM categories WHERE nom='Poissons'),true,3),
('Buffet Complet','Entree + plat au choix + boisson',15.00,(SELECT id FROM categories WHERE nom='Buffet du Jour'),true,1),
('Buffet Famille 4 personnes','Assortiment complet pour 4 personnes',50.00,(SELECT id FROM categories WHERE nom='Buffet du Jour'),true,2),
...
('Kwanga','Pain de manioc',2.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,3),
('Fufu de Manioc',NULL,3.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,4),
('Biere Primus 65cl',NULL,4.00,(SELECT id FROM categories WHERE nom='Boissons'),true,1),
('Biere Turbo King 65cl',NULL,4.00,(SELECT id FROM categories WHERE nom='Boissons'),true,2),
('Jus de Fruit Frais','Mangue, passion, ananas',3.00,(SELECT id FROM categories WHERE nom='Boissons'),true,3),
('Eau Minerale','75cl',2.00,(SELECT id FROM categories WHERE nom='Boissons'),true,4),
('Soda','Coca, Fanta, Sprite 33cl',2.50,(SELECT id FROM categories WHERE nom='Boissons'),true,5)
ON CONFLICT DO NOTHING;
